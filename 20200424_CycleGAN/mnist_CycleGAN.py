...
mnist = keras.datasets.MNIST(root='data', download=True, transform=transform)
...
class Generator(tf.keras.Module):

    # 네트워크 구조
    def __init__(self):
        super(Generator, self).__init__()
        self.main = tf.keras.Sequential(
            tf.keras.layers.Linear(in_features=100, out_features=256),
            nn.LeakyReLU(0.2),
